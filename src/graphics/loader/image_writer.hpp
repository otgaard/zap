...
template <>
bool save_image2D<engine::r8_t>(const engine::pixmap<engine::r8_t>& img, const std::string& path) {
    return 0 != stbi_write_png(path.c_str(), img.width(), img.height(), 1, img.data(), int(img.width()*engine::r8_t::bytesize));
}

template <>
bool save_image2D<engine::rgb888_t>(const engine::pixmap<engine::rgb888_t>& img, const std::string& path) {
    return 0 != stbi_write_png(path.c_str(), img.width(), img.height(), 3, img.data(), int(img.width()*engine::rgb888_t::bytesize));
}

template <>
bool save_image2D<engine::rgba8888_t>(const engine::pixmap<engine::rgba8888_t>& img, const std::string& path) {
